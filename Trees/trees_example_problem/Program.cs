...
// In this example we will simulate a database for student ID's
// While ID's are not 1 to 100, for simplicity we will using these numbers

class Program
{
    static void Main()
    {
       BinarySearchTree studentIDs = new BinarySearchTree();
       ...
       // Now we have to find student with ID 74
       Console.WriteLine();
       studentIDs.Contains(74);
       Console.WriteLine();

       // We created a linked list by default when setting the root to one
       // We need to rebalance by finding the middle then making that the root and 
       // adding the left and right as sub trees
       studentIDs.Balance();
       ...
       // Now if a students were to graduate you remove their ID's

       studentIDs.Remove(34);
       studentIDs.Remove(10);
       studentIDs.Remove(70);
       studentIDs.Remove(53);
       studentIDs.Remove(60);
       studentIDs.Remove(39);
       studentIDs.Remove(8);
       studentIDs.Remove(44);
       studentIDs.Remove(99);
       studentIDs.Remove(50);

       // Tree is balanced but we can now see the new root.
       studentIDs.Balance();
       studentIDs.Inorder();

       // In this example probelm you can quickly find, remove, or add 
       // With such a large amount of data we can quickly    
    }
}